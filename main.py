...
def set_diff(date: str, followers: int, added_names: list[str], removed_names: list[str]):
    diff = Path(DATA_DIR) / 'diff.csv'
...
def main():
    ...
    latest_file = Path(DATA_DIR) / 'latest.json'
    today_file = Path(DATA_DIR) / f'{today}.json'
